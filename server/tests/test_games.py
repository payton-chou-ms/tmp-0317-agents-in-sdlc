...
class TestGamesRoutes(unittest.TestCase):
    ...
    def test_create_game_success(self) -> None:
        ...
        # Arrange: get valid publisher and category IDs
        games = self._get_response_data(self.client.get(self.GAMES_API_PATH))
        publisher_id = games[0]['publisher']['id']
        category_id = games[0]['category']['id']

        payload = {
            "title": "Deploy or Die",
            "description": "Race to deploy your app before the deadline",
    ...
    def test_create_game_missing_field(self) -> None:
        """Test that creating a game with a missing required field returns 400"""
        payload = {
    ...
    def test_create_game_invalid_publisher(self) -> None:
        ...
        payload = {
            "title": "Ghost Publisher",
            "description": "A game with a publisher that does not exist",
            "publisher_id": 9999,
            "category_id": category_id
        }
    ...
    def test_create_game_invalid_category(self) -> None:
        ...
        payload = {
            ...
            "description": "A game with a category that does not exist",
            "publisher_id": publisher_id,
            "category_id": 9999
        }
    ...
    def test_update_game_success(self) -> None:
        ...
        payload = {"title": "Pipeline Panic Updated", "star_rating": 5.0}
    ...
    def test_update_game_invalid_publisher(self) -> None:
        """Test that updating a game with a non-existent publisher returns 404"""
        games = self._get_response_data(self.client.get(self.GAMES_API_PATH))
        game_id = games[0]['id']
    ...
    def test_update_game_validation_error(self) -> None:
        """Test that updating a game with an invalid title returns 400"""
        games = self._get_response_data(self.client.get(self.GAMES_API_PATH))
        game_id = games[0]['id']

        response = self.client.put(
            f'{self.GAMES_API_PATH}/{game_id}',
            data=json.dumps({"title": "X"}),  # too short
            content_type='application/json'
        )
        data = self._get_response_data(response)
